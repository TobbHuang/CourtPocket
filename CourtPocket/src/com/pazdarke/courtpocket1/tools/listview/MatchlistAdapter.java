...
public class MatchlistAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			} else if (str_status.equals("等待对手付款")) {
				holder.tv_billstatus.setTextColor(context.getResources()
						.getColor(R.color.orange));
			} else if (str_status.equals("待验证")||str_status.equals("待评价")) {
				holder.tv_billstatus.setTextColor(context.getResources()
						.getColor(R.color.yellow));
			} else if (str_status.equals("正在寻找对手")	
					|| str_status.equals("重新寻找对手")) {
				holder.tv_billstatus.setTextColor(context.getResources()
			...
			holder.tv_time.setText(time[(Integer) mylist.get(position).get(
					data[4])]);
			...
			String money = (String) mylist.get(position).get(data[6]);
			if (money.equals("0元")) {
				holder.tv_money.setText("暂无");
				holder.tv_money.setTextColor(context.getResources().getColor(R.color.darkGrey));
			} else{
				holder.tv_money.setText(money);
				holder.tv_money.setTextColor(context.getResources().getColor(R.color.blue));
			}
