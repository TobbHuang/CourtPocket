...
public class BookConfirmActivity extends Activity {
	...
	public static Activity instance_bookconfirm;// 用于关闭
	...
	class CreateBillHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			try {
				...
				if (json.getString("GenerateBill").equals("Passcode验证失败")) {
					Data.ISLOGIN = false;
					Toast.makeText(BookConfirmActivity.this, "登录信息过期咯，请重新登录~",
							Toast.LENGTH_SHORT).show();
					progressDialog.dismiss();
					startActivity(new Intent(BookConfirmActivity.this,
							LoginActivity.class));

				} else if (json.getString("GenerateBill").equals("订单生成成功")) {
	...
	class TimeoutHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			Toast.makeText(BookConfirmActivity.this, "网络出问题咩(+﹏+)~",
					Toast.LENGTH_SHORT).show();
			progressDialog.dismiss();
		}
	}
